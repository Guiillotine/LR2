...
///	while (1) { add_cow(cow1, food1, num_cow, num_food); size_arr_cow(cow1, num_cow, num_cow2); }
//	while (1) { add_warehouse(warehouse1, num_warehouse); size_arr_warehouse(warehouse1, num_warehouse, num_warehouse2); }
//	while (1) { add_cowshed(cowshed1, num_cowshed); size_arr_cowshed(cowshed1, num_cowshed, num_cowshed2); }
//	while (1) { add_food(food1, num_food); size_arr_food(food1, num_food, num_food2); }
///	while (1) { add_worker(worker1, num_worker); size_arr_food(worker1, num_worker, num_worker2); }
...
/*	do			//Изменение размера динамичекого массива warehouse1
	...
	//sell_warehouse(warehouse1, num_warehouse);

	/*while (1)			//Изменение размера массива cowshed
	{
